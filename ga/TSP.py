...
def processTour(fileName, numPreLines):
    ...
    for row in range(len(listData)):
        ...
        for col in range(len(listData)):
            ...
            d = math.sqrt((listData[row][0] - listData[col][0]) ** 2 + (listData[row][1] - listData[col][1]) ** 2)
            # print(d)
            r.append(d)
        TSPMatrix.append(r)
        if row % 1000 == 0:
...
pickle.dump(tsp, open("sw24978.p", "wb"))
print(os.path.getsize("sw24978.p"))
test = pickle.load(open("sw24978.p", "rb"))
# print(test)
...
# if len(test) == 29 and len(test[0]) == 29:
#     print("Correctly sized matrix")
